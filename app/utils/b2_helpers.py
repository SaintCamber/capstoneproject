...
def get_upload_url(s3_api_url=os.environ.get("s3_api_url"),account_authorization_token=os.environ.get("account_authorization_token"),bucket_id=os.environ.get("bucketId")):
    headers = {'Authorization': account_authorization_token}
    data = {'bucketId': bucket_id}
    response = requests.post(f'{s3_api_url}/b2api/v2/b2_get_upload_url', headers=headers, json=data)
...
def authorize_account(account_id=os.environ.get('account_id'),application_key_id=os.environ.get("B2_KEY"),application_key=os.environ.get("B2_SECRET")):
