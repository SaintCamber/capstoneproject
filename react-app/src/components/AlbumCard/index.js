...
function AlbumCardContainer({ album }) {
  const history = useHistory();
  const dispatch = useDispatch();

  return (
    <div className="album-card" onClick={() => history.push(`/albums/${album.id}`)}>
      <div className="album-card__art">
        {album.album_art ? (
          <img className="album-card__image" src={album.album_art} alt={`Album art for ${album}`} />
        ) : (
          <div className="album-card__art-placeholder">
